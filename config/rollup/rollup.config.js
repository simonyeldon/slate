...
function configure(pkg, env, target) {
  ...
  const input = `packages/${pkg.name.split('/')[1]}/src/index.ts`
  const deps = []
  ...
  const plugins = [
    ...
    typescript({
      abortOnError: false,
      tsconfig: `./packages/${pkg.name.split('/')[1]}/tsconfig.json`,
    ...
    // modules by default.
    commonjs({
      exclude: [`packages/${pkg.name.split('/')[1]}/src/**`],
    ...
    babel({
      runtimeHelpers: true,
      include: [`packages/${pkg.name.split('/')[1]}/src/**`],
  ...
  if (isUmd) {
    return {
      ...
      output: {
        format: 'umd',
        file: `packages/${pkg.name.split('/')[1]}/${
  ...
  if (isModule) {
    return {
      ...
      output: [
        {
          file: `packages/${pkg.name.split('/')[1]}/${pkg.module}`,
          format: 'es',
          sourcemap: true,
        },
        {
          file: `packages/${pkg.name.split('/')[1]}/${pkg.main}`,
          format: 'cjs',
          exports: 'named',
          sourcemap: true,
        },
